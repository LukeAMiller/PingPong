...
class Callback extends Component {
  async componentDidMount() {
    ...
    fetch(`http://localhost:4321/Players?aud=${auth0Client.getProfile().sub}`)
      .then(matchingEmployee => matchingEmployee.json())
      .then(matchingEmployee => {
        console.log("This is our array of Players that have the current user's aud", matchingEmployee);
        ...
        if (matchingEmployee.length === 0) {
          ...
          // Post it!!
          fetch(`http://localhost:4321/Players`, {
            method: "POST",
            headers: {
